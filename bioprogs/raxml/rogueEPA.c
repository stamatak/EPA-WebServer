...
static void traverse(nodeptr p, nodeptr q, tree *tr, int *branchCounter, double *scores)
{
  double 
    result = 0.0;
  ...
  */
  ...
  if(!isTip(q->number, tr->rdta->numsp))
    {    
      traverse(p, q->next->back, tr, branchCounter, scores);
      traverse(p, q->next->next->back, tr, branchCounter, scores);    
    }    
}
  ...
  for(tips = 1; tips <= tr->mxtips; tips++)    
      ...
      for(i = 0; i < tr->numBranches; i++)
	{
      ...
      */
      ...
      traverse(p, myStart->back, tr, &branchCounter, scores);
